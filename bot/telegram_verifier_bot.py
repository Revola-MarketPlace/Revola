...
BOT_TOKEN           = os.getenv("TELEGRAM_BOT_TOKEN",  "8327143273:AAGL2PIHuJRaGrEPYe40jS1eT_VrrPzAGzI")
...
def handle_buyer_start(chat_id, token, buyer_name):
    ...
    if not result.get("success"):
        error_msg = result.get("message", "Invalid or expired link.")
        send(
            chat_id,
            f"❌ <b>Invalid or Expired Session</b>\n\n"
            f"{error_msg}\n\n"
            f"Please return to the website and start your payment flow again."
        )
        return
...
def handle_callback(callback):
    ...
    if data.startswith("verify:"):
        if not is_admin:
            answer_cb(cb_id, "Unauthorized.", alert=True)
            return

        parts = data.split(":", 2)
        if len(parts) != 3:
            answer_cb(cb_id, "Invalid action.", alert=True)
            return
        ...
        if result.get("success"):
            ...
            answer_cb(cb_id, f"Error: {result.get('message')}", alert=True)
    ...
    # ── Buyer wizard callbacks ──
        ...
        if state:
            ...
            answer_cb(cb_id, "Session expired.", alert=True)
