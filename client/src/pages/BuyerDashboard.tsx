...
const BuyerDashboard: React.FC = () => {
  ...
  const handleSubmitBankRef = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        showToast('Reference submitted successfully! Status is pending verification.', 'success');
        setBankRef('');
        // Refresh
      ...
      showToast(err.response?.data?.message || 'Failed to submit reference.', 'error');
    } finally {
      ...
      <div className="lg:col-span-8 space-y-6">
        {selectedOrder ? (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
            ...
            {selectedOrder.paymentMethod === 'BANK_TRANSFER' && selectedOrder.paymentStatus === 'PENDING_VERIFICATION' && !selectedOrder.refNumber && (
              <div className="p-5 border border-amber-200 bg-amber-50/50 rounded-2xl space-y-4">
                ...
                <div className="text-xs text-amber-800 space-y-1">
                  <p>1. Please transfer <strong>{selectedOrder.total.toLocaleString()} ETB</strong> to the following account:</p>
                  <p>• Commercial Bank of Ethiopia (CBE): <strong>1000554433221</strong> (Account Name: Adama Materials)</p>
                  <p>2. Paste your transaction reference code below to submit it for verification:</p>
                </div>
                <form onSubmit={handleSubmitBankRef} className="flex gap-2 max-w-md">
                  <input
                    type="text"
                    required
                    value={bankRef}
