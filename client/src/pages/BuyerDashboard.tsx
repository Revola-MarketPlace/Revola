...
const BuyerDashboard: React.FC = () => {
  ...
  const handleSubmitBankRef = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bankRef.trim()) return;
    setSubmittingRef(true);

    try {
      const res = await api.post("/payments/submit-reference", {
      ...
      if (res.data.success) {
        showToast(
          "Reference submitted successfully! Status is pending verification.",
          "success",
        );
        setBankRef("");
        // Refresh
      ...
      showToast(
        err.response?.data?.message || "Failed to submit reference.",
        "error",
      );
    } finally {
  ...
  const handleSubmitDispute = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        showToast(
          "Dispute opened. Payout held until administrator review.",
          "success",
        );
      ...
      <div className="lg:col-span-8 space-y-6">
        {selectedOrder ? (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200/60 rounded-2xl">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-xs">
                <div className="flex items-center gap-2">
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${["ASSIGNED", "PICKED_UP", "OUT_FOR_DELIVERY", "DELIVERED"].includes(selectedOrder.deliveryStatus) ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                  >
                    ✓
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${["PICKED_UP", "OUT_FOR_DELIVERY", "DELIVERED"].includes(selectedOrder.deliveryStatus) ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                  >
                    ✓
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${["OUT_FOR_DELIVERY", "DELIVERED"].includes(selectedOrder.deliveryStatus) ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                  >
                    ✓
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${selectedOrder.deliveryStatus === "DELIVERED" ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                  >
                    ✓
                  </span>
            ...
            {/* Bank Transfer Instructions Submission */}
              ...
              !selectedOrder.refNumber && (
                <div className="p-5 border border-amber-200 bg-amber-50/50 rounded-2xl space-y-4">
                  <h4 className="font-bold text-amber-900 text-sm flex items-center gap-1.5">
                    ...
                    Bank Transfer Instructions (CBE / CBE Birr)
                  </h4>
                  <div className="text-xs text-amber-800 space-y-1">
                    ...
                    <p>
                      ...
                      <strong>1000554433221</strong> (Account Name: Adama
                      Materials)
                    </p>
                    <p>
                      2. Paste your transaction reference code below to submit
                      it for verification:
                    </p>
                  </div>
                  <form
                    onSubmit={handleSubmitBankRef}
                  ...
                  >
                    <input
                      ...
                      placeholder="e.g. FT260811xxxx"
                      className="flex-1 px-3 py-2 border border-amber-300 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white text-slate-900"
                    />
                    <button
                      type="submit"
                      disabled={submittingRef}
                      className="bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all disabled:opacity-50 cursor-pointer"
                    >
                      {submittingRef ? "Submitting..." : "Submit Ref"}
                    </button>
                  </form>
                </div>
              )}
            ...
            <div className="space-y-3">
              ...
              <div className="divide-y divide-slate-100">
                {selectedOrder.items.map((item: any) => (
                  <div
                    key={item._id}
                    className="py-3.5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 border-t border-slate-100 pt-6 text-xs text-slate-600">
              ...
              <div className="space-y-1 text-right sm:text-right">
                ...
                <div>
                  Subtotal: {selectedOrder.subtotal.toLocaleString()} ETB
                </div>
                <div>
                  Delivery Fee: {selectedOrder.deliveryFee.toLocaleString()} ETB
                </div>
                <div className="font-extrabold text-sm text-primary-900 mt-1">
                  Total: {selectedOrder.total.toLocaleString()} ETB
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center text-slate-400">
            No order details selected. Click an order from the list to view
            tracking details.
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-4">
            ...
            <form onSubmit={handleSubmitDispute} className="space-y-4">
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-[10px] text-rose-800 leading-relaxed">
                <strong>Attention:</strong> Initiating a dispute immediately
                locks the seller payout in our escrow database. Platform
                administrators will investigate the transaction.
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  Reason
