...
const SellerDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"LISTINGS" | "PAYOUTS">(
    "LISTINGS",
  );
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-slate-900">
              {stats.totalSales.toLocaleString()} ETB
            </span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-slate-900">
              {stats.paidPayout.toLocaleString()} ETB
            </span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-slate-900">
              {stats.eligiblePayout.toLocaleString()} ETB
            </span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-xl font-black text-slate-900">
              {stats.pendingPayout.toLocaleString()} ETB
            </span>
          </div>
        </div>
      </section>
        ...
        {activeTab === "LISTINGS" ? (
          products.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center text-slate-400">
              You haven't listed any materials yet. Click the button above to
              list!
            </div>
          ) : (
              ...
              {products.map((p) => (
                ...
                >
                  <div className="aspect-video bg-slate-100 relative overflow-hidden">
                    ...
                    />
                    <span
                      className={`absolute top-3 left-3 text-[10px] font-bold px-2.5 py-0.5 rounded-full border shadow-sm ${
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-100">
                      <span>
                        Price: <strong>{p.price} ETB</strong>
                      </span>
                      <span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center text-slate-400">
            No payout transactions found. Payouts are spawned when buyers
            checkout items from your listings.
          </div>
        ) : (
                ...
                {payouts.map((p) => (
                  <tr key={p._id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-4 text-slate-400">
                      {p.commissionAmount.toLocaleString()} ETB
                    </td>
                    <td className="p-4 font-black text-slate-900">
                      {p.payoutAmount.toLocaleString()} ETB
                    </td>
                    <td className="p-4">
                      <span
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 max-w-xl w-full shadow-2xl border border-slate-200 space-y-4 my-8">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-900 text-sm">
                Create Usable Material Listing
              </h3>
              <button
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2.5 items-center">
                  ...
                  {uploadedImages.map((url, idx) => (
                    <div
                      key={idx}
                      className="w-16 h-16 rounded-xl overflow-hidden border border-slate-200 relative group"
                    >
                      <img
                        src={url}
                        alt=""
