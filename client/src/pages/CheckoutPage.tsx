...
import { MapPin, CreditCard, ShoppingBag, Landmark, ArrowRight, ArrowLeft } from 'lucide-react';
...
const CheckoutPage: React.FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'TELEBIRR' | 'BANK_TRANSFER'>('BANK_TRANSFER');
  ...
  useEffect(() => {
    ...
    api.post('/orders/estimate-delivery-fee', {
      ...
      .then(res => {
        if (res.data.success) {
          ...
          if (res.data.addresses.length > 0) {
            const def = res.data.addresses.find((a: any) => a.isDefault) || res.data.addresses[0];
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (paymentMethod === 'BANK_TRANSFER' || paymentMethod === 'TELEBIRR') {
          ...
          setGeneratingLink(true);
          try {
            const linkRes = await api.get(`/payments/generate-bot-link/${orderId}`);
            if (linkRes.data.success) {
              setBotDeepLink(linkRes.data.deepLink);
              setBankTransferSuccess({
                orderId,
                orderTotal: linkRes.data.orderTotal || response.data.order.total,
                trackingNumber: linkRes.data.trackingNumber || response.data.order.trackingNumber,
              ...
              showToast('Redirecting to Telegram bot to verify payment...', 'success');
              setTimeout(() => {
      ...
      const msg = error.response?.data?.message || 'Checkout failed. Try again.';
      showToast(msg, 'error');
    } finally {
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 flex-1 flex flex-col items-center">
        <div className="w-full bg-white border border-slate-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-8 py-7 text-white text-center">
            ...
            <p className="text-emerald-100 text-sm mt-1">Now complete your payment via bank transfer</p>
          </div>

          {/* Order Info */}
          <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <div>
              <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Tracking Number</p>
              <p className="text-lg font-extrabold text-slate-900 font-mono">{bankTransferSuccess.trackingNumber}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Amount Due</p>
              <p className="text-2xl font-extrabold text-emerald-700">{bankTransferSuccess.orderTotal?.toLocaleString()} ETB</p>
            </div>
          </div>
          ...
          <div className="px-8 py-6 space-y-6">
            ...
            <div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold text-slate-800">{bankTransferSuccess.bankName}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-400 font-medium">Account Number</span>
                  <span className="text-base font-extrabold text-slate-900 font-mono tracking-widest">{bankTransferSuccess.accountNumber}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-400 font-medium">Account Name</span>
                  <span className="text-sm font-bold text-slate-800 text-right max-w-[55%]">{bankTransferSuccess.accountName}</span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                  <span className="text-xs text-slate-400 font-medium">Amount to Transfer</span>
                  <span className="text-base font-extrabold text-emerald-700">{bankTransferSuccess.orderTotal?.toLocaleString()} ETB</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-sky-50 border border-blue-200 rounded-xl p-5">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-800 text-base">Submit Receipt via Telegram Bot</h3>
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    After transferring, open our Telegram bot to submit your reference number. 
                    Your admin will verify it and you'll receive your <strong>Order ID</strong> instantly.
                  </p>
                </div>
              </div>
              <ol className="text-xs text-slate-600 space-y-1.5 mb-4 list-decimal list-inside">
                <li>Transfer <strong>{bankTransferSuccess.orderTotal?.toLocaleString()} ETB</strong> to the account above</li>
                <li>Tap the button below to open our Telegram bot</li>
                <li>The bot will ask for your transfer reference number</li>
                <li>Admin verifies and you receive your Order ID via the bot</li>
              ...
              {botDeepLink ? (
                ...
                <button
                  onClick={() => handleGenerateBotLink(bankTransferSuccess.orderId)}
                ...
                >
                  {generatingLink ? 'Generating link...' : '🤖 Open Telegram Bot & Submit Receipt'}
                </button>
              )}
            </div>
            ...
            <div className="text-center">
              <button
                onClick={() => navigate(`/buyer-dashboard?order=${bankTransferSuccess.orderId}`)}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 flex flex-col">
      ...
      <div className="flex items-center justify-center gap-4 max-w-lg mx-auto w-full mb-10">
        <div className="flex items-center gap-2">
          <span className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm shadow-md ${step >= 1 ? 'bg-primary-600 text-white' : 'bg-slate-200 text-slate-500'}`}>1</span>
          <span className={`text-sm font-semibold ${step >= 1 ? 'text-primary-900' : 'text-slate-400'}`}>Address</span>
        </div>
        <div className="h-0.5 bg-slate-200 flex-1"></div>
        <div className="flex items-center gap-2">
          <span className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm shadow-md ${step >= 2 ? 'bg-primary-600 text-white' : 'bg-slate-200 text-slate-500'}`}>2</span>
          <span className={`text-sm font-semibold ${step >= 2 ? 'text-primary-900' : 'text-slate-400'}`}>Payment</span>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              {addresses.length > 0 && (
                <div className="space-y-3">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Your Saved Addresses</p>
                    ...
                    {addresses.map((a) => (
                      ...
                      >
                        ...
                        <div className="text-xs space-y-1">
                          <div className="font-bold text-slate-800">{a.title} {a.isDefault && <span className="text-[9px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded font-medium ml-1.5">DEFAULT</span>}</div>
                          <div className="text-slate-500">{a.streetAddress}, {a.subCity}</div>
              ...
              {!selectedAddressId && (
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">New Delivery Address (Adama City)</p>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 mb-1">Sub-City / Kebele</label>
                      <input
                        type="text"
                        value={subCity}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Street Address Details</label>
                    <input
                      type="text"
                      value={streetAddress}
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-sm text-slate-800">Telebirr Mobile Payment</div>
                    <div className="text-xs text-slate-400">Pay securely using your Ethio Telecom Telebirr account.</div>
                  </div>
                </label>
                ...
                >
                  ...
                  <div className="flex-1 flex gap-2.5 items-center">
                    ...
                    <div>
                      <div className="font-bold text-sm text-slate-800">Manual Bank Transfer</div>
                      <div className="text-xs text-slate-400">Transfer funds manually and submit reference. Requires staff verification.</div>
                    </div>
                  </div>
                </label>
              </div>
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
            ...
            {items.map((item) => (
              <div key={item.product._id} className="py-3 flex gap-3 text-xs">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-slate-400 mt-0.5">{item.quantity} x {item.product.price} ETB</div>
                </div>
                <div className="font-bold text-slate-800 self-center">{(item.quantity * item.product.price).toLocaleString()} ETB</div>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-slate-100 pt-4 space-y-3 text-xs">
              ...
              {feeBreakdown && (
                <div className="text-[10px] text-slate-500 space-y-0.5 pt-1.5 border-t border-slate-200/60">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-700">{feeBreakdown.dayLabel} ({feeBreakdown.dayName})</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Quantity ({feeBreakdown.totalQuantity} items):</span>
                    <span className="font-semibold text-slate-700">{feeBreakdown.quantitySurcharge > 0 ? `+${feeBreakdown.quantitySurcharge} ETB bulk fee` : 'Base Qty Included'}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border-t border-slate-100 pt-3 flex justify-between text-sm font-extrabold text-slate-950">
              ...
              <span className="text-primary-900 text-base">{(subtotal + (estimatedFee !== null ? estimatedFee : 100)).toLocaleString()} ETB</span>
            </div>
          </div>
        </div>
      </div>
