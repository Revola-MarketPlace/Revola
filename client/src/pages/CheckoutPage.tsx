...
const CheckoutPage: React.FC = () => {
  ...
  const [deliveryLat, setDeliveryLat] = useState<number>(8.54);
  const [deliveryLng, setDeliveryLng] = useState<number>(39.27);
  ...
  useEffect(() => {
    if (!user || user.role !== "BUYER") {
      showToast(
        "Only BUYER accounts can checkout and place orders.",
        "warning",
      );
        ...
        if (res.data.success) {
          ...
          if (userAddresses.length > 0) {
            ...
            if (
              ...
              def.location.coordinates.length === 2
            ) {
  ...
  const handleLocationPicked = (loc: {
    ...
    showToast(
      `Delivery location pinned at (${loc.latitude.toFixed(4)}, ${loc.longitude.toFixed(4)})`,
      "success",
    );
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showToast(
          "Order created successfully! Redirecting to Chapa Gateway...",
          "success",
        );
                ...
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <div>
                    <span className="font-bold">
                      {hasSelectedGps
                        ? "GPS Pin Configured:"
                        : "Default Adama Center GPS:"}
                    </span>{" "}
                    Lat: {deliveryLat.toFixed(5)}, Lng: {deliveryLng.toFixed(5)}{" "}
                    (Adama Service Area)
                  </div>
                </div>
                <button
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-500 rounded-2xl p-6 space-y-4">
                ...
                <div className="text-xs text-slate-600 space-y-2 pt-3 border-t border-emerald-200">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      256-bit encrypted checkout verified server-side with
                      Chapa.
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                    <span>
                      Supports Visa, Mastercard, Telebirr, CBE Birr, and Amole.
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-1">
                <div className="font-bold text-slate-800 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-emerald-600" /> Delivery
