...
const CartPage: React.FC = () => {
  const { user } = useAuth();
  const { items, subtotal, loading, updateQuantity, removeFromCart } =
    useCart();
  ...
  const handleQtyChange = async (
    ...
    if (targetQty > stockLimit) {
      showToast(
        `Cannot exceed available seller stock limit of ${stockLimit} units.`,
        "warning",
      );
      return;
    }
      ...
      {items.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm flex flex-col items-center justify-center flex-1 max-w-2xl mx-auto w-full my-12">
          ...
          <p className="text-sm text-slate-500 mt-2 max-w-sm">
            It looks like you haven't added any usable materials to your cart
            yet. Browse our catalog to find items!
          </p>
          <Link
            to="/products"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-primary-700 font-extrabold text-sm mt-3">
            ...
            <Link
              to="/products"
              className="inline-flex items-center gap-1 text-sm font-bold text-primary-600 hover:underline pt-2"
            >
          ...
          <div className="lg:col-span-4 bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
            ...
            {user && user.role !== "BUYER" ? (
              <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-amber-900 text-xs">
                <strong>Logged in as {user.role}:</strong> Purchasing materials
                is restricted to Buyer accounts. Please sign in as a Buyer to
                checkout.
              </div>
            ) : (
              <button
            ...
            <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-xs text-slate-500 leading-relaxed">
              <strong>Escrow Guarantee:</strong> Payment is securely held.
              Delivery fee is set manually per order by staff depending on size
              and address details inside Adama City.
            </div>
          </div>
        </div>
      )}
