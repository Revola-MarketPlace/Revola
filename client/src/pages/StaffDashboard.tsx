...
const StaffDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "DELIVERIES" | "FEES" | "BANK_VERIFICATION"
  >("DELIVERIES");
  ...
  const handleSetDeliveryFee = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        showToast(
          `Delivery fee set to ${deliveryFee} ETB. Order total updated.`,
          "success",
        );
  ...
  const deliveriesNeedingFee = deliveries.filter(
    (d) => d.fee === 0 && d.order?.orderStatus !== "CANCELLED",
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 flex flex-col gap-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage platform deliveries, bank confirmations, and logistics.
        </p>
      </div>
                  ...
                  {deliveries.map((d) => (
                    <tr key={d._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-4 max-w-xs truncate">
                        {d.order?.deliveryAddress.streetAddress},{" "}
                        {d.order?.deliveryAddress.subCity}
                      </td>
                      <td className="p-4 font-semibold">{d.fee} ETB</td>
                      <td className="p-4 uppercase">
                        {d.order?.paymentMethod.replace("_", " ")}
                      </td>
                      <td className="p-4">
                        <span
              ...
              {deliveriesNeedingFee.map((d) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {submittingFee && selectedDeliveryId === d._id
                        ? "Saving..."
                ...
                >
                  ...
                  <div className="space-y-3 pt-2">
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="e.g. CBE Transfer confirmed on ledger"
                        value={
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 rounded-xl transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-bold py-2 rounded-xl transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-1"
                      >
                        <X className="w-4 h-4" />
                        Reject / Decline
                      </button>
