...
const CatalogPage: React.FC = () => {
  ...
  const conditions: string[] = (searchParams.get("condition") || "")
    .split(",")
    .filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (conditions && conditions.length > 0)
      query.set("condition", conditions.join(","));
  ...
  const toggleCondition = (cond: string) => {
    const nextParams = new URLSearchParams(searchParams);
    let current: string[] = (searchParams.get("condition") || "")
      .split(",")
      .filter(Boolean);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
            Usable Materials Catalog
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Found {total} approved materials in Adama City
          </p>
        </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 flex-1">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl border border-slate-200 p-4 h-80 animate-pulse space-y-4"
                >
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center text-slate-500 flex flex-col items-center justify-center flex-1">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Try tweaking your keywords or category selections
              </p>
              <button
                onClick={handleClearFilters}
                  ...
                  >
                    <div className="aspect-square bg-slate-100 relative overflow-hidden">
                      <img
                        src={
                          p.images?.[0] ||
                          getMaterialFallbackImage(p.name, p.category?.name)
                        }
                        alt={p.name}
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="font-bold text-slate-900 text-sm line-clamp-1 group-hover:text-primary-600 transition-colors mt-0.5">
                          {p.name}
                        </h4>
