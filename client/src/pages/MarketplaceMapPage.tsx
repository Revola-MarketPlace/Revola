...
const ADAMA_CENTER_COORDS: [number, number] = [8.54, 39.27];
...
const isInsideAdamaServiceArea = (lat: number, lng: number): boolean => {
  let inside = false;
  for (
    let i = 0, j = ADAMA_SERVICE_AREA.length - 1;
    i < ADAMA_SERVICE_AREA.length;
    j = i++
  ) {
...
const MarketplaceMapPage: React.FC = () => {
  ...
  const [typeFilter, setTypeFilter] = useState<
    "ALL" | "MARKETPLACE_SELLER" | "ADMIN_MANAGED" | "OSM_EXTERNAL"
  >("ALL");

  // Real Live Device/Browser GPS Location state
  const [buyerLocation, setBuyerLocation] = useState<DetectedLocation | null>(
    null,
  );
  ...
  const filteredPlaces = useMemo(() => {
    return places.filter((p) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchMaterials = p.materials?.some((m) =>
          m.toLowerCase().includes(q),
        );
        const matchCategory = p.category?.toLowerCase().includes(q);
        if (!matchTitle && !matchAddress && !matchMaterials && !matchCategory)
          return false;
      }
      return true;
    });
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.on("click", (e: L.LeafletMouseEvent) => {
        ...
        if (isInside) {
          showToast(
            `Location set (${clickedLat}, ${clickedLng}) — Inside Adama`,
            "success",
          );
        } else {
          showToast(
            `Location set (${clickedLat}, ${clickedLng}) — Outside Adama`,
            "info",
          );
        }
      });
  ...
  const renderUserMarker = useCallback((loc: DetectedLocation) => {
    ...
    userMarkerRef.current = L.marker([loc.lat, loc.lng], {
      ...
      .bindPopup(popupHtml)
      .bindTooltip(
        `You are here (${isInside ? "Inside Adama" : "Outside Adama"})`,
        {
          permanent: false,
          direction: "top",
          offset: [0, -22],
  ...
  const processPosition = (pos: GeolocationPosition, isFirstFix = false) => {
    ...
    if (isFirstFix) {
      if (isInside) {
        showToast(
          `GPS Position: (${lat.toFixed(4)}, ${lng.toFixed(4)}) — Inside Adama City!`,
          "success",
        );
      } else {
        showToast(
  ...
  const handleCenterUser = () => {
    if (buyerLocation && mapInstanceRef.current) {
      mapInstanceRef.current.flyTo([buyerLocation.lat, buyerLocation.lng], 15, {
        duration: 1,
      });
    }
  };
  ...
  const handleGetDirections = async (place: PlaceItem) => {
    ...
    try {
      ...
      if (
        ...
        showToast(
          `Route mapped: ${res.data.distanceKm} km (~${res.data.durationMin} mins)`,
          "success",
        );
      }
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col lg:flex-row w-full min-h-[calc(100vh-5rem)] h-full bg-slate-100 relative">
            ...
            <div className="p-3 bg-amber-50/90 border border-amber-200 rounded-2xl space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div className="text-[11px] text-amber-900 leading-tight">
                  <strong>Location permission is required</strong> to verify
                  Adama service area and calculate delivery distances.
                </div>
              </div>
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-extrabold flex items-center gap-1.5">
                  <span
                    className={`w-2 h-2 rounded-full ${isLiveTracking ? "bg-emerald-500 animate-pulse" : "bg-blue-600"}`}
                  ></span>
                ...
                {buyerLocation.isInside ? (
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Inside
                    Adama City
                  </span>
                ) : (
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-rose-100 text-rose-800 border border-rose-300 flex items-center gap-1">
                    <ShieldAlert className="w-3 h-3 text-rose-600" /> Outside
                    Service Area
                  </span>
                )}
              </div>
              ...
              <div className="text-[11px] font-mono bg-white/95 p-2.5 rounded-xl border border-slate-200/80 shadow-xs flex flex-col gap-0.5 text-slate-800">
                ...
                <div className="flex justify-between items-center pt-1 border-t border-slate-100 text-[10px]">
                  ...
                  <span
                    className={`font-bold ${buyerLocation.accuracy > 150 ? "text-amber-600" : "text-slate-700"}`}
                  >
                    ±{Math.round(buyerLocation.accuracy)} meters{" "}
                    {buyerLocation.accuracy > 150
                      ? "(Approximate / Network)"
                      : ""}
                  </span>
                </div>
              ...
              {!buyerLocation.isInside ? (
                <div className="p-2 bg-rose-100/80 rounded-xl border border-rose-200 text-[11px] text-rose-900 leading-tight">
                  <strong>Service Notice:</strong> Your current location (
                  {buyerLocation.lat.toFixed(4)}, {buyerLocation.lng.toFixed(4)}
                  ) is outside the Adama service area. AdaMaterials is currently
                  available only in Adama City. Click on the map to place your
                  pin in Adama.
                </div>
              ) : (
                <div className="text-[10px] text-emerald-800 font-medium">
                  ✔ You are inside the Adama service area. Full marketplace
                  shopping & delivery enabled.
                </div>
              )}
      ...
      <div className="flex-1 h-[52vh] lg:h-[calc(100vh-5rem)] min-h-[450px] relative z-10 order-1 lg:order-2 bg-slate-200">
        ...
        <div className="absolute top-4 left-4 z-[400] flex flex-wrap items-center gap-2">
          ...
          >
            <Navigation
              className={`w-4 h-4 text-blue-600 ${gpsLoading ? "animate-spin" : ""}`}
            />
          ...
          >
            <Radio
              className={`w-4 h-4 ${isLiveTracking ? "text-white animate-pulse" : "text-slate-600"}`}
            />
            ...
            <div className="p-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {activePlace.placeType === "OSM_EXTERNAL" && (
                    <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-800 flex items-center gap-1.5">
                      <Globe className="w-3.5 h-3.5 text-slate-600" /> Community
                      OSM
                    </span>
                  )}
              ...
              <div className="mt-3 space-y-1 text-xs text-slate-600">
                ...
                {activePlace.distanceKm !== null &&
                  placeWithDistanceKm(activePlace) && (
                    <div className="flex items-center gap-2 text-blue-700 font-bold">
                      <Car className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" />
                      <span>
                        {activePlace.distanceKm} km from your location
                      </span>
                    </div>
                  )}
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2">
                ...
                {activePlace.placeType === "MARKETPLACE_SELLER" && (
                  ...
                  >
                    <span>
                      Products ({activePlace.availableProductsCount || 0})
                    </span>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-extrabold text-white">
                  Route to: {routeInfo.targetTitle}
                </div>
                <div className="text-[11px] text-slate-300 flex items-center gap-2 mt-0.5">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {routeInfo.durationMin} mins drive
                  </span>
                </div>
              </div>
            </div>
