...
const RegisterPage: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    const newUser = await registerAuth(data.name, data.email, data.password, data.role);
    if (newUser) {
    ...
    <div className="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl border border-slate-200">
        <div className="text-center">
          <span className="inline-flex items-center justify-center bg-primary-600 text-white w-12 h-12 rounded-xl shadow-md text-2xl font-bold mb-4">A</span>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedRole === 'BUYER' ? 'border-primary-500 bg-primary-50/50 text-primary-900' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'}`}>
                  <input type="radio" value="BUYER" {...register('role')} className="sr-only" />
                  <span className="text-sm font-bold">Buyer</span>
                  <span className="text-[10px] text-center mt-1">Browse and purchase materials</span>
                </label>
                <label className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedRole === 'SELLER' ? 'border-primary-500 bg-primary-50/50 text-primary-900' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'}`}>
                  <input type="radio" value="SELLER" {...register('role')} className="sr-only" />
                  <span className="text-sm font-bold">Seller</span>
                  <span className="text-[10px] text-center mt-1">List and sell usable waste</span>
                </label>
              </div>
            </div>
          ...
          {selectedRole === 'SELLER' && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5 text-xs text-amber-800">
              ...
              <p>
                <strong>Seller Notice:</strong> To maintain platform quality, newly registered seller accounts must be approved by a platform administrator before listing public items.
              </p>
            </div>
          )}
