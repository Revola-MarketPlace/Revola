...
import { Users, LayoutGrid, FileCheck, AlertOctagon, Landmark, ShieldCheck, Plus, Trash2, X, Eye, Truck, RotateCcw } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'METRICS' | 'USERS' | 'PRODUCTS' | 'BANK_VERIFICATION' | 'DELIVERIES' | 'CONFIG' | 'DISPUTES' | 'AUDIT_LOGS'>('METRICS');
                  ...
                  {p.submissionType === 'RECEIPT_IMAGE' ? (
                    <div className="p-3 bg-purple-50 border border-purple-200 rounded-xl space-y-1">
                      ...
                      <div className="text-xs text-purple-800 font-medium">
                        Buyer submitted a receipt screenshot via Telegram.
                        Check the admin Telegram chat for the attached image.
                      </div>
                      {p.receiptImage && (
                      ...
                      {(p.botPaymentMethod || p.bankName) && (
                        <div className="text-[10px] text-purple-600 mt-1">
                          Method: <strong>{p.botPaymentMethod === 'TELEBIRR' ? 'Telebirr' : 'Bank Transfer'}</strong>
                          {p.bankName && <> &mdash; Bank: <strong>{p.bankName}</strong></>}
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl space-y-1">
                      <div className="text-[10px] text-amber-500 font-bold uppercase">Submitted Reference Code</div>
                      <div className="font-mono text-sm font-bold text-amber-900">{p.refNumber || 'EMPTY'}</div>
                      {(p.botPaymentMethod || p.bankName) && (
