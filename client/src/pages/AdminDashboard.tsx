            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {bankPayments.map((p) => (
                ...
                >
                  ...
                  {p.submissionType === "RECEIPT_IMAGE" ? (
                    <div className="p-3 bg-purple-50 border border-purple-200 rounded-xl space-y-1">
                      <div className="text-[10px] text-purple-600 font-bold uppercase flex items-center gap-1">
                        <span>📷</span> Screenshot Submission
                      </div>
                      <div className="text-xs text-purple-800 font-medium">
                        Buyer submitted a receipt screenshot via Telegram. Check
                        the admin Telegram chat for the attached image.
                      </div>
                      {p.receiptImage && (
                        <div className="text-[10px] text-purple-500 mt-0.5 font-mono break-all">
                          Telegram file_id: {p.receiptImage}
                        </div>
                      )}
                      {(p.botPaymentMethod || p.bankName) && (
                        <div className="text-[10px] text-purple-600 mt-1">
                          Method:{" "}
                          <strong>
                            {p.botPaymentMethod === "TELEBIRR"
                              ? "Telebirr"
                              : "Bank Transfer"}
                          </strong>
                          {p.bankName && (
                            <>
                              {" "}
                              &mdash; Bank: <strong>{p.bankName}</strong>
                            </>
                          )}
                        </div>
                      )}
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 rounded-xl transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-1 text-xs"
                      >
