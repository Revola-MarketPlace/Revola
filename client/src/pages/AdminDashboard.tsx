...
const AdminDashboard: React.FC = () => {
  ...
  const handleResetAllStats = async () => {
    if (
      !window.confirm(
        "Are you sure you want to reset all statistics and operational data? This will clear all orders, payments, payouts, and notifications for clean testing.",
      )
    )
      return;
    try {
      ...
      showToast(
        err.response?.data?.message || "Failed to reset statistics.",
        "error",
      );
    }
  };
  ...
  const handleProductReview = async (
    ...
    try {
      ...
      showToast(
        err.response?.data?.message || "Failed to review product.",
        "error",
      );
    }
  };
  ...
  const handleAddMaterial = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/admin/material-types", {
        name: matName,
        description: matDesc,
      });
  ...
  const handleOpenEditPlace = (place: any) => {
    ...
    setPlaceMaterials(
      Array.isArray(place.materials) ? place.materials.join(", ") : "",
    );
    setPlaceAddress(place.address || "");
    setPlacePhone(place.phone || "");
    setPlaceLat(
      place.location?.coordinates?.[1]
        ? String(place.location.coordinates[1])
        : "8.5400",
    );
    setPlaceLng(
      place.location?.coordinates?.[0]
        ? String(place.location.coordinates[0])
        : "39.2700",
    );
  ...
  const handleSaveMapPlace = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.message || "Failed to save map place.",
        "error",
      );
    } finally {
  ...
  const handleTogglePlaceStatus = async (place: any) => {
    try {
      await api.put(`/admin/map-places/${place._id}`, {
        isActive: !place.isActive,
      });
      showToast(
        `Place ${place.isActive ? "deactivated" : "activated"} successfully.`,
        "success",
      );
      fetchAdminMapPlaces();
    } catch (err: any) {
      showToast(
        err.response?.data?.message || "Failed to update place status.",
        "error",
      );
    }
  };
  ...
  const handleDeleteMapPlace = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this map place?"))
      return;
    try {
      ...
      showToast(
        err.response?.data?.message || "Failed to delete map place.",
        "error",
      );
    }
  };
  ...
  // Filter lists
  const pendingProductReviews = allProducts.filter(
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Platform management, approvals, configurations, and dispute
            arbitrations.
          </p>
        </div>
        <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
                ...
                <span className="text-2xl font-black text-slate-900 mt-1 block">
                  {stats.finance.revenue.toLocaleString()} ETB
                </span>
              </div>
              <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
                <span className="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">
                ...
                <span className="text-2xl font-black text-slate-900 mt-1 block">
                  {stats.finance.deliveryFees.toLocaleString()} ETB
                </span>
              </div>
              <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
                <span className="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">
                ...
                <span className="text-2xl font-black text-slate-900 mt-1 block">
                  {stats.finance.payouts.PAID.toLocaleString()} ETB
                </span>
              </div>
              <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
                <span className="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">
                    ...
                    <tr key={u._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-4 flex gap-2">
                        ...
                        {u.role !== "ADMIN" && (
                          <button
                            onClick={() =>
                              handleToggleUserSuspension(u._id, !u.isActive)
                            }
                  ...
                  <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        Buyer: {p.order?.buyer?.name || "Buyer"} (
                        {p.order?.buyer?.email})
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <span className="font-black text-sm text-primary-900">
                    ...
                    {(p.botPaymentMethod || p.bankName || p.provider) && (
                      <div className="flex justify-between items-center text-xs text-slate-600">
                        ...
                        <span className="font-semibold text-slate-800">
                          {p.botPaymentMethod || p.provider || "BANK_TRANSFER"}{" "}
                      ...
                      <div className="pt-2 border-t border-slate-200/60">
                        <div className="flex justify-between items-center mb-1.5">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const resolved =
                                p.receiptImage?.startsWith("data:") ||
                                p.receiptImage?.startsWith("blob:")
                                  ? p.receiptImage
                        ...
                        {p.receiptImage.startsWith("http") ||
                        p.receiptImage.startsWith("/uploads") ||
                        p.receiptImage.startsWith("data:") ? (
                          <div
                            onClick={() => {
                  ...
                  <div className="space-y-3 pt-1">
                    <div>
                      ...
                      <input
                        ...
                        value={
                          selectedPaymentId === p._id ? verificationNotes : ""
                        }
                        onChange={(e) => {
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={
                          processingVerification && selectedPaymentId === p._id
                        }
            ...
            >
              <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <span className="font-bold text-slate-900 text-sm">
                  Payment Receipt Full Inspection
                </span>
                <button
                  onClick={() => setPreviewImageUrl(null)}
              ...
              {disputes.map((d) => (
                ...
                >
                  ...
                  <div className="text-xs space-y-2">
                    <div>
                      Reason:{" "}
                      <strong className="text-slate-800">{d.reason}</strong>
                    </div>
                    <p className="text-slate-500 italic bg-slate-50 p-3 rounded-xl border border-slate-100">
                      "{d.description}"
                    </p>
                  </div>
                  ...
                  {selectedDispute?._id === d._id && (
                    ...
                    >
                      <div>
                        ...
                        >
                          <option value="BUYER_REFUND">
                            Buyer Refund (Cancels Seller Payout)
                          </option>
                          <option value="SELLER_PAYOUT_RELEASED">
                            Seller Payout Released (Confirms Order)
                          </option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-6">
              ...
              <form onSubmit={handleAddCategory} className="flex gap-2 text-xs">
                ...
                <button
                  type="submit"
                  className="bg-primary-600 hover:bg-primary-700 text-white font-bold px-3 py-1.5 rounded-lg flex items-center justify-center cursor-pointer"
                >
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
            <div>
              <h3 className="text-lg font-bold text-slate-900">
                Delivery Tracking System Management
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Admin control panel to view, track, and directly update delivery
                tracking statuses across all marketplace orders.
              </p>
            </div>
                    ...
                    {deliveries.map((delivery) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4 text-slate-600 text-xs">
                          {delivery.order?.deliveryAddress?.subCity || "Adama"},{" "}
                          {delivery.order?.deliveryAddress?.streetAddress || ""}
                        </td>
                        <td className="py-3.5 px-4 text-slate-600 text-xs">
                        ...
                        <td className="py-3.5 px-4 text-right whitespace-nowrap">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const note = prompt(
                                `Enter tracking note for status update to ${newStatus}:`,
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Manage verified local material depots, salvage yards, and
                collection points within the Adama City service area.
              </p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <span className="font-bold text-xs text-slate-700">
                All Configured Places ({mapPlaces.length})
              </span>
              <span className="text-[11px] text-slate-400">
                🟡 Verified Depot • ⚪ External Community
              </span>
            </div>

            {mapPlaces.length === 0 ? (
              <div className="p-12 text-center text-xs text-slate-400">
                No map places configured yet. Click "Add New Depot / Place" to
                create one.
              </div>
            ) : (
              <div className="overflow-x-auto">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveMapPlace} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block font-semibold text-slate-600 mb-1">
                  Materials Available (comma-separated)
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 max-h-36 overflow-y-auto border border-slate-100 p-2.5 rounded-xl bg-slate-50">
                  {staffPermissionsList.map((perm) => (
                    <label
                      key={perm}
                      className="flex items-center gap-1.5 text-[10px] text-slate-600 cursor-pointer"
                    >
                      <input
                        type="checkbox"
