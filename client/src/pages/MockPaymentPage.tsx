...
import { AlertOctagon, CheckCircle2, XCircle, ShieldAlert, Sparkles } from 'lucide-react';
      ...
      <div className="max-w-md w-full space-y-6 bg-slate-950 border border-slate-800 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        {status === 'IDLE' && (
          <div className="space-y-6 pt-6">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-black text-white mt-3">Simulate Payment Gateway</h2>
              <p className="text-xs text-slate-400 mt-1.5">This screen mimics a hosted Chapa/Telebirr payment redirection.</p>
            </div>

            <div className="p-4 bg-slate-900 border border-slate-850 rounded-xl space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-slate-500">Transaction ID:</span>
                <span className="font-mono text-slate-300 font-bold">{transactionId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Provider:</span>
                <span className="uppercase text-slate-300 font-bold">{getProvider()}</span>
              </div>
              <div className="border-t border-slate-850 pt-2 flex justify-between text-sm">
                <span className="text-slate-400">Amount Due:</span>
                <span className="text-accent-400 font-black">{Number(amount).toLocaleString()} ETB</span>
              </div>
            </div>
        ...
        {status === 'SUCCESS' && (
          <div className="text-center space-y-6 pt-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-400">The simulated webhook updated the order status to CONFIRMED.</p>
            </div>
            <button
        ...
        {status === 'FAILED' && (
          <div className="text-center space-y-6 pt-6">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-white">Payment Declined</h2>
              <p className="text-xs text-slate-400">The transaction was reported as cancelled or failed.</p>
            </div>
            <button
              onClick={() => navigate('/buyer-dashboard')}
