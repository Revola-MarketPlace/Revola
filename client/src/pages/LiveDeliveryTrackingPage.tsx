...
const LiveDeliveryTrackingPage: React.FC = () => {
    ...
    const initMap = async () => {
      ...
      if (order.deliveryAddress?.latitude && order.deliveryAddress?.longitude) {
        ...
        destinationMarkerRef.current = L.marker(
          ...
          .bindTooltip(`📦 Delivery: ${order.deliveryAddress.streetAddress}`, {
            permanent: false,
          })
          .addTo(map);
      }
  ...
  useEffect(() => {
    ...
    const isActiveDelivery = ["ASSIGNED", "OUT_FOR_DELIVERY"].includes(
      deliveryStatus,
    );
  ...
  const isTrackingActive = ["ASSIGNED", "OUT_FOR_DELIVERY"].includes(
    deliveryStatus,
  );
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isTrackingActive ? (
              ...
              <span
                className={`px-3 py-1.5 rounded-xl text-xs font-bold ${STATUS_COLORS[deliveryStatus] || "text-slate-600 bg-slate-100"}`}
              >
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              {/* Progress Steps */}
              {["PENDING", "ASSIGNED", "OUT_FOR_DELIVERY", "DELIVERED"].map(
                (step, idx) => {
                  const isCompleted =
                    [
                      "PENDING",
                      ...
                      "OUT_FOR_DELIVERY",
                      "DELIVERED",
                    ].indexOf(deliveryStatus) >= idx;
                  const isCurrent = deliveryStatus === step;

                  return (
                    <div
                      key={step}
                      className={`flex items-start gap-3 ${idx < 3 ? "pb-4" : ""}`}
                    >
                      <div
                      ...
                      <div
                        className={`flex-1 ${idx < 3 ? "border-l-2 pl-3 ml-[-28px] mt-7 mb-[-16px]" : ""}`}
                      >
                        <p
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-3">
            ...
            <div className="text-xs text-slate-600 space-y-0.5">
              ...
              <p>
                {order.deliveryAddress.subCity}, {order.deliveryAddress.city}
              </p>
          ...
          {courierLocation && (
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 space-y-2">
              ...
              <p className="text-xs text-blue-700">
                Lat: {courierLocation.latitude.toFixed(5)}, Lng:{" "}
                {courierLocation.longitude.toFixed(5)}
              </p>
              {lastUpdated && (
          ...
          {!isTrackingActive && deliveryStatus !== "DELIVERED" && (
            <div className="p-5 border-t border-slate-100 text-center">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs font-semibold text-amber-700">
                ...
                <span>
                  Awaiting courier assignment — live GPS tracking will begin
                  automatically
                </span>
              </div>
            </div>
