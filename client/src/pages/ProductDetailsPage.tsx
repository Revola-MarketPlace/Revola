...
const ProductDetailsPage: React.FC = () => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-slate-500 mt-2">
          The product you are looking for does not exist or is pending review.
        </p>
        <Link
          to="/products"
          className="inline-flex items-center gap-1 mt-6 text-primary-600 font-bold hover:underline"
        >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1">
      ...
      <Link
        to="/products"
        className="inline-flex items-center gap-1 text-sm font-semibold text-slate-500 hover:text-primary-600 mb-8 transition-colors"
      >
        <ChevronLeft className="w-4 h-4" />
        Back to Catalog
      </Link>
          ...
          <div className="aspect-square bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden relative">
            ...
            {product.quantity === 0 && (
              <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center">
                <span className="bg-rose-500 text-white font-extrabold text-lg px-6 py-2.5 rounded-xl shadow-lg border border-rose-400">
                  OUT OF STOCK
                </span>
              </div>
            )}
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className="flex items-center gap-0.5 text-amber-500">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-4.5 h-4.5 ${i < Math.round(avgRating) ? "fill-amber-500" : "text-slate-300"}`}
                  />
                ))}
              </div>
          ...
          <div className="border-t border-b border-slate-200/80 py-4 space-y-3">
            <div className="flex items-center gap-2.5 text-sm text-slate-600">
              ...
              <span>
                Location: <strong>{product.location.subCity}, Adama</strong>
              </span>
            </div>
            <div className="flex items-center gap-2.5 text-sm text-slate-600">
              <Tag className="w-4.5 h-4.5 text-slate-400 flex-shrink-0" />
              <span>
                Material: <strong>{product.materialType?.name}</strong>
              </span>
            </div>
            <div className="flex items-center gap-2.5 text-sm text-slate-600">
              ...
              <span>
                ...
                <strong
                  className={
                    product.quantity > 0 ? "text-slate-800" : "text-rose-500"
                  }
                >
          ...
          {user && user.role !== "BUYER" ? (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900 text-xs flex flex-col gap-1">
              ...
              <span>
                Only registered <strong>BUYER</strong> accounts are authorized
                to purchase materials. If you wish to purchase, please sign in
          ...
          <div className="bg-slate-100 border border-slate-200 p-4 rounded-2xl flex flex-col gap-1 text-xs text-slate-500">
            ...
            <span>
              Escrow Guarantee: Payout is held until courier delivery completes.
            </span>
          </div>
        </div>
      </div>
      ...
      <section className="border-t border-slate-200/80 pt-12 mb-16">
        ...
        {reviews.length === 0 ? (
          <p className="text-slate-400 text-sm">
            No reviews yet for this product. Be the first to review after
            purchase!
          </p>
        ) : (
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-0.5 text-amber-500 mt-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-4 h-4 ${i < r.rating ? "fill-amber-500" : "text-slate-300"}`}
                        />
                      ))}
                    </div>
                  </div>
            ...
            {related.map((p) => (
              ...
              >
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <h4 className="font-bold text-slate-900 text-xs line-clamp-1 group-hover:text-primary-600 transition-colors">
                    {p.name}
                  </h4>
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
                    <span className="text-primary-700 font-extrabold text-xs">
                      {p.price} ETB
                    </span>
                  </div>
                </div>
