...
const isInsideAdama = (lat: number, lng: number): boolean => {
  let inside = false;
  for (
    let i = 0, j = ADAMA_POLYGON.length - 1;
    i < ADAMA_POLYGON.length;
    j = i++
  ) {
...
const LocationPickerModal: React.FC<LocationPickerModalProps> = ({
    ...
    try {
      ...
      if (response.ok) {
        ...
        const display = suburb
          ? `${suburb}, ${city}`
          : data.display_name?.split(",").slice(0, 2).join(",") || city;
        setDetectedAddress(display);
      }
    } catch {
      // Fallback if offline or network rate limit
      setDetectedAddress(
        isValid ? "Adama City Service Area" : "Outside Adama Area",
      );
    } finally {
      setGeocoding(false);
    }
  ...
  const handleGetCurrentGps = () => {
    if (!navigator.geolocation) {
      setGpsError(
        "Geolocation is not supported by your browser. You can click on the map to place your pin in Adama.",
      );
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setGpsLoading(false);
        setGpsError(
          `GPS permission was not granted or location unavailable (${err.message}). Please click on the map to pin your location in Adama.`,
        );
      },
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="relative flex-1 min-h-[380px] bg-slate-100">
          ...
          <div className="absolute top-4 right-4 z-[400] flex flex-col sm:flex-row gap-2">
            ...
            >
              <Navigation
                className={`w-4 h-4 text-amber-600 ${gpsLoading ? "animate-spin" : ""}`}
              />
            ...
            <div className="flex flex-wrap items-center gap-2">
              {isValid ? (
                <div className="flex items-center gap-2 text-xs font-semibold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200">
                  ...
                  <span>
                    Inside Adama Service Area ({lat.toFixed(4)},{" "}
                    {lng.toFixed(4)})
                    {detectedAddress && (
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-xs font-semibold text-rose-700 bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-200">
                    ...
                    <span>
                      Outside Adama Service Area ({lat.toFixed(4)},{" "}
                      {lng.toFixed(4)})
                      {detectedAddress && (
                  ...
                  <span className="text-[11px] text-slate-500">
                    AdaMaterials operates exclusively in Adama City. Click{" "}
                    <strong>"Center Adama"</strong> or drag the pin inside the
                    orange service area boundary.
                  </span>
                </div>
              )}
            </div>
