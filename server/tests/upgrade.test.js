...
const {
  ...
  calculateDistanceKm,
} = require("../src/config/serviceArea");
...
describe("Major Feature Upgrade Test Suite", () => {
  ...
  it("Geofence: Inside Adama coordinates are accepted", () => {
    ...
    assert.strictEqual(isLocationInAdamaServiceArea(8.542, 39.278), true);
    // Kebele 02, Adama (8.5460, 39.2710)
    assert.strictEqual(isLocationInAdamaServiceArea(8.546, 39.271), true);
    // Wonji Road Junction (8.5180, 39.2820)
    assert.strictEqual(isLocationInAdamaServiceArea(8.518, 39.282), true);
  });
  ...
  it("Distance Calculation: Accurately computes distance between coordinates", () => {
    const dist = calculateDistanceKm(8.542, 39.278, 8.546, 39.271);
  ...
  it("User Model: Seller banking details are private (select: false) in standard queries", async () => {
    ...
    assert.strictEqual(
      adminFetched.sellerProfile.bankAccountNumber,
      "1000999888777",
    );
    assert.strictEqual(
      adminFetched.sellerProfile.bankAccountHolder,
  ...
  it("Order Model: Supports deliveryLocation GeoJSON and stores coordinates safely", async () => {
    ...
    const order = await Order.create({
      ...
      deliveryLocation: {
        type: "Point",
        coordinates: [39.278, 8.542],
      },
    });

    assert.strictEqual(order.paymentMethod, "CHAPA");
    assert.strictEqual(order.deliveryLocation.coordinates[0], 39.278);
    assert.strictEqual(order.deliveryLocation.coordinates[1], 8.542);
  });
