...
const login = async (email, password = "Password123") => {
  ...
  assert.strictEqual(
    res.status,
    200,
    `Login failed for ${email}: ${JSON.stringify(res.data)}`,
  );
...
test.describe("Full Integration Test Suite", () => {
  ...
  test.before(async () => {
    ...
    const mat = await MaterialType.create({
      name: "Test Material",
      description: "Test",
    });
    ...
    const staffFin = await User.create({
      name: "Staff Finance",
    ...
    const staffLog = await User.create({
      name: "Staff Logistics",
      email: "staffl@test.com",
      password: "Password123",
      role: "STAFF",
      staffPermissions: [
        "MANAGE_DELIVERIES",
  ...
  test("Auth: Staff with wrong permission cannot verify payments", async () => {
    ...
    const res = await api(
      ...
      { paymentId: "fake", status: "PAID" },
      staffLogisticsCookie,
    );
    assert.strictEqual(
      res.status,
      403,
      "Staff without VERIFY_PAYMENTS must be blocked",
    );
  });
  ...
  test("Auth: Staff with wrong permission cannot set delivery fees", async () => {
    ...
    const res = await api(
      "POST",
      "/deliveries/set-fee",
      { deliveryId: "fake", fee: 100 },
      staffFinanceCookie,
    );
    assert.strictEqual(
      res.status,
      403,
      "Staff without SET_DELIVERY_FEES must be blocked",
    );
  });
  ...
  test("Products: Draft products are hidden from public catalog", async () => {
    ...
    assert.strictEqual(
      found,
      false,
      "Draft product must not appear in public catalog",
    );
  });
  ...
  test("Checkout: Non-CHAPA payment methods (MOCK, BANK_TRANSFER, TELEBIRR, INVALID) are rejected", async () => {
    ...
    for (const invalidMethod of [
      ...
      assert.strictEqual(
        res.status,
        400,
        `Payment method ${invalidMethod} must be rejected`,
      );
      assert.ok(
        res.data.message.toLowerCase().includes("invalid payment method"),
        `Message should mention invalid method for ${invalidMethod}`,
      );
    }
  });
  ...
  test("Checkout: Creates multi-seller order with per-seller payouts using CHAPA", async () => {
    ...
    assert.strictEqual(
      res.status,
      201,
      `Checkout failed: ${JSON.stringify(res.data)}`,
    );
    ...
    assert.strictEqual(
      payouts.length,
      2,
      "Two payouts must be created — one per seller",
    );
    ...
    payouts.forEach((p) => {
      assert.strictEqual(
        p.status,
        "PENDING",
        "Payout must be PENDING at checkout",
      );
    });

    payout1Id = payouts
      .find((p) => p.seller.toString() === seller1Id)
      ?._id.toString();
    payout2Id = payouts
      .find((p) => p.seller.toString() === seller2Id)
      ?._id.toString();
  });
  ...
  test("Payment: Mock webhook processes payment and confirms order", async () => {
    ...
    assert.strictEqual(
      res.status,
      200,
      `Webhook failed: ${JSON.stringify(res.data)}`,
    );
    ...
    assert.strictEqual(
      updated.status,
      "PAID",
      "Payment must be marked PAID after webhook",
    );
    ...
    assert.strictEqual(
      order.orderStatus,
      "CONFIRMED",
      "Order must be CONFIRMED after payment",
    );
  });
  ...
  test("Payment: Webhook is idempotent (duplicate call is ignored)", async () => {
    ...
    assert.strictEqual(res.status, 200);
    assert.ok(
      res.data.message?.includes("Already processed"),
  ...
  test("Payment: Payouts remain PENDING (not eligible) before delivery", async () => {
    ...
    payouts.forEach((p) => {
      assert.strictEqual(
        p.status,
        "PENDING",
        "Payouts must stay PENDING until delivery",
      );
    });
  });
  ...
  test("Delivery: Staff with SET_DELIVERY_FEES can set delivery fee", async () => {
    ...
    assert.strictEqual(
      res.status,
      200,
      `Set fee failed: ${JSON.stringify(res.data)}`,
    );
    ...
    assert.strictEqual(
      order.deliveryFee,
      150,
      "Delivery fee must update order",
    );
  });
  ...
  test("Delivery: Staff updates status to DELIVERED — payouts become ELIGIBLE", async () => {
    ...
    await api(
      ...
      "/deliveries/assign",
      {
        deliveryId,
        staffId: (
          await User.findOne({ email: "staffl@test.com" })
        )._id.toString(),
      },
      staffLogisticsCookie,
    );
    ...
    assert.strictEqual(
      res.status,
      200,
      `Update delivery status failed: ${JSON.stringify(res.data)}`,
    );
    ...
    payouts.forEach((p) => {
      assert.strictEqual(
        p.status,
        "ELIGIBLE",
        "Payout must become ELIGIBLE after delivery",
      );
    });
  });
  ...
  test("Payout: Staff can process ELIGIBLE payout", async () => {
    ...
    assert.strictEqual(
      res.status,
      200,
      `Process payout failed: ${JSON.stringify(res.data)}`,
    );
    ...
    assert.strictEqual(
      payout.status,
      "PAID",
      "Payout must be PAID after processing",
    );
  });
  ...
  test("Payout: Cannot process already-PAID payout (invalid state transition)", async () => {
    ...
    assert.strictEqual(
      res.status,
      400,
      "Duplicate payout processing must be rejected",
    );
  });
  ...
  test("State machine: Cannot change status of a DELIVERED order to PENDING_PAYMENT", async () => {
    ...
    assert.strictEqual(
      res.status,
      400,
      "Invalid state regression must be rejected",
    );
  });
  ...
  test("Webhook: Missing tx_ref returns 400", async () => {
    const res = await api("POST", "/payments/webhook/chapa", {
      status: "success",
    });
  ...
  test("Commission: Per-seller payout amount correctly deducts commission", async () => {
    ...
    for (const p of payouts) {
      const expectedCommission = parseFloat(
        (p.amount * p.commissionRate).toFixed(10),
      );
  ...
  test("Role Permission: Seller cannot add items to cart or checkout (HTTP 403)", async () => {
    ...
    assert.strictEqual(
      cartRes.status,
      403,
      "Seller must not be allowed to add items to cart",
    );
    ...
    assert.strictEqual(
      checkoutRes.status,
      403,
      "Seller must not be allowed to checkout",
    );
  });
  ...
  test("Role Permission: Admin cannot add items to cart or checkout (HTTP 403)", async () => {
    ...
    assert.strictEqual(
      cartRes.status,
      403,
      "Admin must not be allowed to add items to cart",
    );
    ...
    assert.strictEqual(
      checkoutRes.status,
      403,
      "Admin must not be allowed to checkout",
    );
  });
  ...
  test("Role Permission: Buyer cannot create seller product listings (HTTP 403)", async () => {
    const res = await api(
      ...
      "/products",
      {
        ...
        description: "Attempted by Buyer",
        price: 100,
        quantity: 1,
        category: categoryId,
        materialType: materialTypeId,
    ...
    assert.strictEqual(
      res.status,
      403,
      "Buyer must be forbidden from creating seller listings",
    );
  });
});
