...
test.describe('Full Integration Test Suite', () => {
  ...
  test('Checkout: Invalid payment method is rejected', async () => {
    ...
    const res = await api('POST', '/orders/checkout', {
      ...
      deliveryAddress: { streetAddress: '1 Test St', city: 'Adama', subCity: 'Test', phoneNumber: '0911000000' },
    }, buyerCookie);
    assert.strictEqual(res.status, 400, 'Invalid payment method must be rejected');
    assert.ok(res.data.message.toLowerCase().includes('invalid payment method'));
  });
  ...
  test('Checkout: Creates multi-seller order with per-seller payouts', async () => {
    ...
    const res = await api('POST', '/orders/checkout', {
      paymentMethod: 'MOCK',
      deliveryAddress: { streetAddress: '1 Test St', city: 'Adama', subCity: 'Bole', phoneNumber: '0911000000' },
    }, buyerCookie);
  ...
  test('Checkout: Buyer cannot exceed available stock', async () => {
    ...
    const res = await api('POST', '/orders/checkout', {
      paymentMethod: 'MOCK',
      deliveryAddress: { streetAddress: '2 Test Ave', city: 'Adama', subCity: 'Test', phoneNumber: '0911000000' },
    }, buyer2Cookie);
