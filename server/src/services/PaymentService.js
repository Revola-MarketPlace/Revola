...
class ChapaProvider extends PaymentProvider {
  ...
  async initializePayment(order, callbackUrl) {
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'Chapa Payment Gateway Initialization Failed.');
    }
  }
  ...
  async verifyPayment(transactionId) {
    ...
    try {
      ...
      if (response.data?.status === 'success') {
        ...
        if (d.status === 'success') {
          return { status: 'PAID', transactionId: d.tx_ref, amount: parseFloat(d.amount) };
        }
      }
      return { status: 'FAILED', transactionId };
    } catch (error) {
...
class TelebirrProvider extends PaymentProvider {
  ...
  async initializePayment(order, callbackUrl) {
    ...
    if (process.env.NODE_ENV === 'production' && (!this.apiKey || !this.secret)) {
      throw new Error('Telebirr API credentials (TELEBIRR_API_KEY, TELEBIRR_SECRET) are required in production.');
    }
    const paymentUrl = `${process.env.CLIENT_URL || 'http://localhost:5173'}/mock-payment/${txRef}?amount=${order.total}`;
    return { success: true, transactionId: txRef, paymentUrl };
  }
  ...
  async verifyPayment(transactionId) {
    ...
    console.warn('Telebirr real-API verification not implemented. Defaulting to PENDING_VERIFICATION.');
