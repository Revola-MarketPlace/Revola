...
function extractChapaErrorMessage(error) {
  ...
  if (respData) {
    ...
    if (typeof respData.error === "string" && respData.error.trim())
      return respData.error.trim();
    if (typeof respData.data === "string" && respData.data.trim())
      return respData.data.trim();
  }
...
class ChapaProvider extends PaymentProvider {
  ...
  async initializePayment(order, callbackUrl) {
    ...
    if (!key) {
      if (process.env.NODE_ENV === "production") {
        throw new Error(
          "CHAPA_SECRET_KEY is missing in server/.env. Please configure your Chapa API Key (e.g. CHASECK_TEST-...).",
        );
      }
      const txRef = `TX-MOCK-${Date.now()}-${order._id}`;
      const paymentUrl = `${process.env.CLIENT_URL || "http://localhost:5173"}/mock-payment/${txRef}?amount=${order.total}`;
  ...
  async verifyPayment(transactionId) {
    ...
    try {
      ...
      return {
        ...
        transactionId,
        message:
          response.data?.message || "Payment verification failed on Chapa",
      };
    } catch (error) {
      console.error(
        "[Chapa Verify Error]:",
...
class BankTransferProvider extends PaymentProvider {
  async initializePayment(order, callbackUrl) {
    ...
    return {
      ...
      paymentUrl: "",
      instructions:
        "Please transfer to Commercial Bank of Ethiopia and upload receipt.",
    };
  }
