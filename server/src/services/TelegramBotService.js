...
class TelegramBotService {
  constructor() {
    this.token =
      ...
      "8327143273:AAGL2PIHuJRaGrEPYe40jS1eT_VrrPzAGzI";
  ...
  async getAdminChatIds() {
    ...
    try {
      ...
      const adminChatsFile = path.join(
        __dirname,
        "../../../bot/admin_chats.json",
      );
      ...
      console.error(
        "[TelegramBot] Could not read admin_chats.json:",
        err.message,
      );
    }
  ...
  async notifyAdminNewReceipt(
    ...
    if (adminChatIds.length === 0) {
      console.log(
        `[TelegramBot] No admin chats registered — skipping admin alert for payment ${paymentId}`,
      );
      return;
    }
    ...
    const methodStr =
      botPaymentMethod === "TELEBIRR" ? "📱 Telebirr" : "🏦 Bank Transfer";
    const bankStr = bankName ? ` (${bankName})` : "";
    const isImageSubmission =
      submissionType === "RECEIPT_IMAGE" || submissionType === "BOTH";
    ...
    const buttons = [
      [
        {
          text: "✅ Approve (PAID)",
          callback_data: `verify:PAID:${paymentId}`,
        },
        {
          text: "❌ Reject (FAILED)",
          callback_data: `verify:FAILED:${paymentId}`,
        },
      ],
    ];
