      ...
      socket.on("join_delivery_tracking", async ({ deliveryId, orderId }) => {
        try {
          ...
          if (deliveryId) {
            ...
            delivery = await Delivery.findOne({ order: orderId }).populate(
              "order",
            );
          }
          ...
          const isBuyer =
            order && order.buyer.toString() === socket.user._id.toString();
          const isAssignedCourier =
            delivery.assignedStaff &&
            delivery.assignedStaff.toString() === socket.user._id.toString();
          ...
          if (!isBuyer && !isAssignedCourier && !isAdminOrStaff) {
            return socket.emit("error_message", {
              message: "Unauthorized to track this delivery",
            });
          }
          ...
          socket.emit("delivery_state", {
            ...
            trackingActive: ["ASSIGNED", "OUT_FOR_DELIVERY"].includes(
              delivery.status,
            ),
        ...
        async ({ deliveryId, latitude, longitude }) => {
          try {
            ...
            if (!isAssignedCourier && !isAdmin) {
              return socket.emit("error_message", {
                message: "Only the assigned courier can broadcast GPS updates",
              });
            }
            ...
            this.io
              ...
              .emit("courier_location_update", locationData);

            // 2. Throttled Database write (at most once every 10 seconds per delivery)
            const lastWrite =
              this.lastDbWriteMap.get(delivery._id.toString()) || 0;
