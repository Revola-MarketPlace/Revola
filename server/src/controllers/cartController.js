...
exports.addToCart = asyncHandler(async (req, res, next) => {
  ...
  if (!product || product.approvalStatus !== "APPROVED") {
    return next(
      new AppError("Product not found or not available for purchase.", 404),
    );
  }

  if (product.quantity < qty) {
    return next(
      new AppError(`Only ${product.quantity} items available in stock.`, 400),
    );
  }
  ...
  if (existingItemIndex > -1) {
    ...
    if (product.quantity < newQty) {
      return next(
        new AppError(
          `Only ${product.quantity} items available in stock. Cannot add more.`,
          400,
        ),
      );
    }
  ...
  const subtotal = populatedCart.items.reduce(
    (sum, item) => sum + item.product.price * item.quantity,
    0,
  );
