...
exports.generateBotLink = asyncHandler(async (req, res, next) => {
  ...
  if (order.paymentMethod !== 'BANK_TRANSFER') {
    return next(new AppError('This order does not use Bank Transfer.', 400));
  }
...
exports.botLinkValidate = asyncHandler(async (req, res, next) => {
  ...
  if (!token || !telegramChatId) {
    return next(new AppError('Token and Telegram chat ID are required.', 400));
  }

  const tokenHash = hashToken(token);

  // Find the payment matching this token hash and still within expiry window
  ...
  if (!payment) {
    return next(new AppError('Invalid or expired bot link. Please return to the website and request a new link.', 401));
  }

  const order = payment.order;
  const buyer = order.buyer;

  // If already linked to a different chat_id, reject (prevent token hijacking)
  if (payment.buyerTelegramChatId && payment.buyerTelegramChatId !== String(telegramChatId)) {
    return next(new AppError('This link has already been used from a different Telegram account.', 403));
  }

  // Register the buyer's Telegram chat_id
  ...
  res.status(200).json({
    ...
    alreadySubmitted: Boolean(payment.refNumber),
  });
});
...
exports.botSubmitReceipt = asyncHandler(async (req, res, next) => {
  const { token, telegramChatId, refNumber, submissionType, receiptImage, botPaymentMethod, bankName } = req.body;

  if (!token || !telegramChatId || (!refNumber && !receiptImage)) {
    return next(new AppError('Token, Telegram chat ID, and either reference number or receipt screenshot are required.', 400));
  }
  ...
  const payment = await Payment.findOne({
    botToken: tokenHash,
    buyerTelegramChatId: String(telegramChatId), // Must match registered chat_id
    botTokenExpiry: { $gt: new Date() },
  }).select('+botToken').populate({
    path: 'order',
  ...
  if (payment.status === 'PENDING_VERIFICATION' && (payment.refNumber || payment.receiptImage)) {
...
exports.submitBankTransferDetails = asyncHandler(async (req, res, next) => {
  ...
  if (order.paymentMethod !== 'BANK_TRANSFER') {
    return next(new AppError('This order does not use bank transfer payment method.', 400));
  }
