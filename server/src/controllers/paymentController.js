...
async function notifyAdminStaff(title, message, relatedId) {
  try {
    ...
    console.error(
      "[notifyAdminStaff] Error creating admin notifications:",
      err.message,
    );
  }
}
...
exports.generateBotLink = asyncHandler(async (req, res, next) => {
  ...
  if (!["BANK_TRANSFER", "TELEBIRR"].includes(order.paymentMethod)) {
    return next(
      new AppError("This order does not use Bank Transfer or Telebirr.", 400),
    );
  }
  ...
  if (!botUsername) {
    return next(
      new AppError("Could not reach Telegram API to generate bot link.", 502),
    );
  }
  ...
  res.status(200).json({
    ...
    bankDetails: {
      ...
      accountName:
        process.env.BANK_ACCOUNT_NAME || "Adama Materials Marketplace PLC",
    },
  });
});
...
exports.botLinkValidate = asyncHandler(async (req, res, next) => {
  ...
  if (!payment || !payment.order) {
    return next(
      new AppError(
        "No pending orders found. Please place an order on the website first.",
        404,
      ),
    );
  }
  ...
  res.status(200).json({
    ...
    alreadySubmitted: Boolean(
      payment.refNumber && payment.refNumber !== "SCREENSHOT_UPLOAD",
    ),
  });
});
...
exports.botSubmitReceipt = asyncHandler(async (req, res, next) => {
  ...
  if (!telegramChatId || (!refNumber && !receiptImage)) {
    return next(
      new AppError(
        "Telegram chat ID, and either reference number or receipt screenshot are required.",
        400,
      ),
    );
  }
  ...
  if (
    ...
    botPaymentMethod === "TELEBIRR" &&
    refNumber &&
    refNumber.trim().length !== 10
  ) {
    return next(
      new AppError(
        "Telebirr transaction reference number must be exactly 10 characters long.",
        400,
      ),
    );
  }
  ...
  if (!payment || !payment.order) {
    return next(
      new AppError(
        "Invalid token or session expired. Please use a fresh link from the website.",
        401,
      ),
    );
  }
  ...
  if (
    ...
    (payment.refNumber || payment.receiptImage) &&
    payment.refNumber !== "SCREENSHOT_UPLOAD"
  ) {
  ...
  payment.submissionType =
    submissionType || (receiptImage ? "RECEIPT_IMAGE" : "REF_NUMBER");
  ...
  const refInfo = receiptImage
    ? `Screenshot attached (${refNumber || "Image"})`
    : `ref: ${refNumber}`;
  ...
  TelegramBotService.notifyAdminNewReceipt(
    ...
    console.error("[TelegramBot] Admin alert error:", err.message),
  );
...
exports.botGetActiveSession = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    botPaymentMethod:
      payment.botPaymentMethod ||
      (payment.provider === "TELEBIRR" ? "TELEBIRR" : "BANK_TRANSFER"),
...
// ─────────────────────────────────────────────────────────────────────────────
// 1. Buyer submits bank transfer reference via the WEBSITE (legacy / fallback)
//    This route still works but the preferred flow is via the bot.
// ─────────────────────────────────────────────────────────────────────────────
exports.submitBankTransferDetails = asyncHandler(async (req, res, next) => {
  ...
  if (!orderId || !refNumber) {
    return next(
      new AppError(
        "Order ID and bank transfer reference number are required.",
        400,
      ),
    );
  }
  ...
  const payment = await Payment.findOne({ order: order._id });
  if (!payment)
    return next(new AppError("Associated payment record not found.", 404));
  ...
  TelegramBotService.notifyAdminNewReceipt(
    ...
    console.error("[TelegramBot] Telegram notification error:", err.message),
  );

  res.status(200).json({
    success: true,
    message:
      "Bank transfer reference submitted successfully. Our staff will verify it shortly.",
    order,
  });
});
...
exports.verifyPaymentManual = asyncHandler(async (req, res, next) => {
  ...
  if (!paymentId || !status) {
    return next(
      new AppError("Payment ID and verification status are required.", 400),
    );
  }

  if (!["PAID", "FAILED"].includes(status)) {
    return next(
      new AppError("Invalid verification status. Must be PAID or FAILED.", 400),
    );
  }
  ...
  if (payment.status === "PAID") {
    return res
      .status(200)
      .json({ success: true, message: "Already verified as PAID.", payment });
  }
  ...
  if (status === "PAID") {
    ...
    if (payment.buyerTelegramChatId) {
      TelegramBotService.notifyBuyerPaymentApproved(
        payment.buyerTelegramChatId,
        order,
      ).catch((err) =>
        console.error(
          "[TelegramBot] Buyer approval notify error:",
          err.message,
        ),
      );
    }
...
exports.verifyOnlinePayment = asyncHandler(async (req, res, next) => {
  ...
  if (!transactionId || !provider) {
    return next(
      new AppError(
        "Transaction reference and provider name are required.",
        400,
      ),
    );
  }
  ...
  if (!payment)
    return next(new AppError("Payment transaction reference not found.", 404));
  ...
  if (result.status === "PAID") {
    ...
    if (result.amount && Math.abs(result.amount - payment.amount) > 1) {
      console.error(
        `[SECURITY] Amount mismatch on ${transactionId}: expected ${payment.amount}, got ${result.amount}`,
      );
      ...
      return next(
        new AppError(
          "Payment amount mismatch detected. Payment rejected for security.",
          400,
        ),
      );
    }
...
exports.handleWebhook = asyncHandler(async (req, res, next) => {
  ...
  if (!txRef) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid payload: tx_ref is required" });
  }
  ...
  if (payment.status === "PAID") {
    return res
      .status(200)
      .json({ success: true, status: "PAID", message: "Already processed" });
  }
