...
exports.createDispute = asyncHandler(async (req, res, next) => {
  ...
  if (!orderId || !reason || !description) {
    return next(
      new AppError("Order ID, reason, and description are required.", 400),
    );
  }
  ...
  if (
    ["PENDING_PAYMENT", "CANCELLED", "REFUNDED"].includes(order.orderStatus)
  ) {
    return next(
      new AppError(
        `Cannot open a dispute for an order in ${order.orderStatus} status.`,
        400,
      ),
    );
  }
  ...
  if (existingDispute) {
    return next(
      new AppError("A dispute is already active for this order.", 400),
    );
  }
  ...
  res.status(201).json({
    success: true,
    message:
      "Dispute opened successfully. A marketplace administrator will investigate.",
    dispute,
  });
});
...
exports.resolveDispute = asyncHandler(async (req, res, next) => {
  ...
  if (!disputeId || !decision || !adminNotes) {
    return next(
      new AppError(
        "Dispute ID, decision, and administrator notes are required.",
        400,
      ),
    );
  }
  ...
  if (decision === "BUYER_REFUND") {
    ...
    order.orderStatus =
      order.deliveryStatus === "DELIVERED" ? "DELIVERED" : "CONFIRMED";
    await order.save();

    // Release payout
    const targetStatus =
      order.deliveryStatus === "DELIVERED" ? "ELIGIBLE" : "PENDING";
    await Payout.updateMany(
      { order: order._id, seller: dispute.seller },
