...
exports.approveSeller = asyncHandler(async (req, res, next) => {
  ...
  await Notification.create({
    ...
    message:
      "Your seller account has been approved. You can now list materials.",
    type: "SYSTEM",
  });
...
exports.reviewProduct = asyncHandler(async (req, res, next) => {
  ...
  await Notification.create({
    user: product.seller,
    title:
      status === "APPROVED" ? "Product Approved! 🎉" : "Product Rejected ⚠️",
    message:
      status === "APPROVED"
...
exports.resetStats = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message:
      "All marketplace statistics, orders, payments, payouts, and notifications have been reset to zero for clean testing!",
  });
});
