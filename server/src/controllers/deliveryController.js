...
exports.setDeliveryFee = asyncHandler(async (req, res, next) => {
  ...
  if (!deliveryId || isNaN(numericFee) || numericFee < 0) {
    return next(
      new AppError(
        "Delivery ID and a valid non-negative fee are required.",
        400,
      ),
    );
  }
...
exports.updateDeliveryStatus = asyncHandler(async (req, res, next) => {
  ...
  if (
    req.user.role === "STAFF" &&
    delivery.assignedStaff?.toString() !== req.user._id.toString() &&
    !req.user.staffPermissions.includes("MANAGE_DELIVERIES")
  ) {
...
exports.getDeliveries = asyncHandler(async (req, res, next) => {
  ...
  const deliveries = await Delivery.find(query)
    .populate(
      "order",
      "trackingNumber orderStatus paymentMethod total subtotal deliveryAddress",
    )
