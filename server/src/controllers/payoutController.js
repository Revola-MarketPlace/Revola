...
exports.processPayout = asyncHandler(async (req, res, next) => {
  ...
  if (!payoutId || !transactionRef) {
    return next(
      new AppError(
        "Payout ID and bank transaction reference are required.",
        400,
      ),
    );
  }
  ...
  if (payout.status !== "ELIGIBLE" && payout.status !== "FAILED") {
    return next(
      new AppError(
        `Cannot process payout in current state: ${payout.status}. Only ELIGIBLE or FAILED payouts can be processed.`,
        400,
      ),
    );
  }
