...
exports.getProductDetails = asyncHandler(async (req, res, next) => {
  ...
  const isSeller = req.user && req.user._id.toString() === product.seller._id.toString();
...
exports.submitForApproval = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: 'Product submitted for approval. An administrator will review it shortly.',
    product,
  });
});
