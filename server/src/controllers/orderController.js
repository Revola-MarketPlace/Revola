...
const MARKETPLACE_COMMISSION_RATE = PaymentService.getCommissionRate
  ? PaymentService.getCommissionRate()
  : parseFloat(process.env.MARKETPLACE_COMMISSION_RATE) || 0.1;
...
exports.checkout = asyncHandler(async (req, res, next) => {
  ...
  if (!deliveryAddress || !paymentMethod) {
    return next(
      new AppError("Delivery address and payment method are required.", 400),
    );
  }

  // Validate payment method — TELEBIRR and BANK_TRANSFER (Telegram Bot flow)
  const ALLOWED_PAYMENT_METHODS = ["TELEBIRR", "BANK_TRANSFER"];
  if (!ALLOWED_PAYMENT_METHODS.includes(paymentMethod)) {
    return next(
      new AppError(
        "Invalid payment method. Allowed methods: Telebirr and Bank Transfer.",
        400,
      ),
    );
  }
  ...
  // Double-check stock availability and approval status (stock is decremented later upon payment verification)
  ...
  if (paymentMethod !== "BANK_TRANSFER") {
    try {
      ...
      const payInit = await PaymentService.initialize(
        order,
        paymentMethod,
        callbackUrl,
      );
      ...
      // We don't roll back order creation here, but let the buyer retry payment from order tracking page
    }
  }
...
exports.getOrderDetails = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role === "SELLER" && !isAdminOrStaff) {
    ...
    // Seller shouldn't see delivery address of the buyer in full or other details if restricted, but let's keep basic item list
  }
...
exports.updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  if (current === "DELIVERED" && !["DISPUTED", "COMPLETED"].includes(status)) {
    return next(
      new AppError(
        "Delivered orders can only transition to DISPUTED or COMPLETED.",
        400,
      ),
    );
  }
  ...
  if (
    ...
    return next(
      new AppError(
        `Cannot change status of a finished order (${current}).`,
        400,
      ),
    );
  }
  ...
  if (status === "DELIVERED") {
    ...
    await Delivery.updateMany(
      { order: order._id },
      { $set: { status: "DELIVERED", notes: note || "Delivered" } },
    );
    ...
    if (deliveredPayment?.buyerTelegramChatId) {
      TelegramBotService.notifyBuyerDelivered(
        deliveredPayment.buyerTelegramChatId,
        order,
      ).catch((err) =>
        console.error("[TelegramBot] Delivery notify error:", err.message),
      );
    }
  }
