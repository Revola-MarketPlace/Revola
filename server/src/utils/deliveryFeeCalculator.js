 ...
 */
function calculateDynamicDeliveryFee({
  address,
  totalQuantity = 1,
  date = new Date(),
}) {
  ...
  if (/01|02|03|04|center|central|piazza|stadium/i.test(locationStr)) {
    ...
    /11|12|13|14|industrial|park|wonji|expressway|outskirt/i.test(locationStr)
  ) {
    ...
    breakdown: {
      ...
      locationZone:
        distanceFee === 70
          ? "Central Adama Zone"
          : distanceFee === 180
            ? "Industrial & Outskirts Zone"
            : "Mid-Range Adama Zone",
    },
  };
}
