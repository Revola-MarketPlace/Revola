...
>>>>>>> 8a6504d (feat: update 37 demo product selling images with verified high quality assets)

    for (const p of products) {
      if (uniqueImagesByProduct[p.name]) {
        p.images = [uniqueImagesByProduct[p.name]];
        await p.save();
      }
    }
